...
bool WellState::add_rates(rd_file_view_type *rst_view, int well_nr) {
...
int WellState::get_lgr_well_nr(const rd_file_view_type *file_view) {
    int well_nr = -1;
...
void WellState::add_connections(const rd_file_view_type *rst_view,
...
void WellState::add_global_connections(const rd_file_view_type *rst_view,
                                       int well_nr) {
...
void WellState::add_LGR_connections(const rd_grid_type *grid,
    ...
    */
        if (lgr_view) {
            ...
            int well_nr = get_lgr_well_nr(lgr_view);
            if (well_nr >= 0)
                add_connections(lgr_view, grid_name, lgr_index + 1, well_nr);
        }
    }
}
...
bool WellState::add_MSW(rd_file_view_type *rst_view, int well_nr,
...
std::shared_ptr<WellState> WellState::read_wells_in_restart(
    ...
    return read_wells_in_restart(rd_file_get_active_view(rd_file), grid,
                                 report_nr, global_well_nr,
                                 load_segment_information);
}

std::shared_ptr<WellState> WellState::read_wells_in_restart(
    rd_file_view_type *file_view, const rd_grid_type *grid, int report_nr,
    int global_well_nr, bool load_segment_information) {
